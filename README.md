...
A quick and dirty ops dashboard for neo4j.

Requires that the server is running JMX stats with the [Jolokia JVM Agent](https://jolokia.org/agent/jvm.html)

## Neo4j Setup

* Download Jolokia 1.5.0 or later.
* Start neo4j with the following in `neo4j.conf`:

```
dbms.jvm.additional=-javaagent:/path/to/jolokia-1.5.0/agents/jolokia-jvm.jar=config=/path/to/jolokia.conf
```

The `jolokia.conf` file needs only contain:
```
cors.allow-origin=*
```

After setup, you should be able to see jolokia running:

`curl http://localhost:8778/jolokia/list/`

## Habanero Quick Setup
